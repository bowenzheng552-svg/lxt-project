...
interface MapFn {
  (n: number, start: number, stop: number, start2: number, stop2: number): number;
}

const mapFn: MapFn = function (n, start, stop, start2, stop2) {
...
class AsciiFilter {
  ...
  constructor(renderer: THREE.WebGLRenderer, { fontSize, fontFamily, charset, invert }: {
    fontSize?: number; fontFamily?: string; charset?: string; invert?: boolean;
  } = {}) {
...
class CanvAscii {
  ...
  updateRotation() {
    const x = mapFn(this.mouse.y, 0, this.height, 0.5, -0.5);
    const y = mapFn(this.mouse.x, 0, this.width, -0.5, 0.5);
